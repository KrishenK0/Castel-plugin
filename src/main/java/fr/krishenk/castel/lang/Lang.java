...
//    TAX_GUILD_KICKED_ANNOUNCE("{$es}%guilds_guild_name% {$e}guild has been kicked out of {$es}%guilds_nation_name% {$e}nation due to not being able to pay the taxes{$sep}: {$es}%guilds_fancy_bank% {$sep}< {$es}$%tax%", 1, 2),
    ...
    COMMAND_REQUIRE_INVITES_DESCRIPTION("{$s}Set the guild invitation rule.", 1, 3),
    COMMAND_REQUIRE_INVITES_USAGE("{$usage}requireinvites {$es}%true/false%{$sep}", 1, 3),
    COMMAND_REQUIRE_INVITES_ON("{$s}Require invite is now &aON{$s}. An invitation is needed to join the guild.", 1, 3),
    COMMAND_REQUIRE_INVITES_OFF("{$s}Require invite is now {$e}OFF{$s}. Everyone can join the guild.", 1, 3),
...
//            "%turrets%\n" +
    ...
    COMMAND_CANCEL_DESCRIPTION("{$s}Cancel an invitation send to a player.", 1, 2),
    COMMAND_CANCEL_USAGE("{$usage}rp cancel {$p}<name>", 1, 2),
    COMMAND_CANCEL_NO_INVITE("{$e}The player is not invited.{$e}", 1, 2),
    COMMAND_CANCEL_INVITATION_CANCELLED("{$p}The invitation of {$es}%receiver%{$p} has been cancelled.", 1, 2),
